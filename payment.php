...
$result_set=mysqli_query($conn1,$query);
$count=mysqli_num_rows($result_set);
      if(!$result_set){
   die("query is failed".mysqli_error($conn1));
}
if($count==0)
{
...
while($row = mysqli_fetch_array($re))
  ...
  $value = mysqli_query($conn1,"UPDATE  bank set Account_Balance='{$row['Account_Balance']}'-'{$amount}' where Account_No= '{$accountno}';");
  
  $sql=mysqli_query($conn2,"INSERT INTO reservation(fname,lname,idno,country,city,phone,price,duration,total,date,roomid,roomno)
VALUES
...
$query1 = "SELECT * FROM bank where Fname= '{$tra}';";
 $result_set=mysqli_query($conn1,$query1);
      if(!$result_set){
   die("query is failed".mysqli_error($conn1));
}
if(mysqli_num_rows($result_set)>0)
{
$result1 = mysqli_query($conn1,"SELECT * FROM bank where Fname= '{$tra}';");
while($row1 = mysqli_fetch_array($result1))
  {
   $value = mysqli_query($conn1,"UPDATE  bank set Account_Balance='{$row1['Account_Balance']}'+'{$amount}' where Fname= '{$tra}';");

   }
$value = mysqli_query($conn2,"UPDATE  rooms set status='reserved' where roomid= '{$room_id}';");   	  
}}}}}	   
else
   {
...
<font color="white" face="arial" size="2px">SRM &copy;&nbsp;2018&nbsp;Copy Right Reserved</font></td>
</tr>
</table>
</center>
</body>
